...
export class Curve {
  ...
  drawParallelCurve(svg, vshift) {
    var i,
      n = this.countOfSegments() * XypicConstants.interpolationResolution;
  ...
  drawParallelDottedCurve(svg, spacing, vshift) {
    var px = 1 / XypicGlobalContext.measure.em,
      hpx = px / 2;
    var sp = px + spacing;
    ...
    if (n >= 0) {
      ...
      for (i = 0; i <= n; i++) {
        ...
        svg.createSvgElement("circle", {
          cx: XypicGlobalContext.measure.em2px(x + dc),
          cy: -XypicGlobalContext.measure.em2px(y + ds),
          r: 0.12,
          fill: "currentColor",
        });
        svg.createSvgElement("circle", {
    ...
    if (len >= wave) {
      ...
      switch (variant) {
        case "3":
          ...
          d1 =
            "M" +
            XypicGlobalContext.measure.em2px(p.x + nx) +
            "," +
            XypicGlobalContext.measure.em2px(-p.y - ny);
          d2 =
            "M" +
            XypicGlobalContext.measure.em2px(p.x) +
            "," +
            XypicGlobalContext.measure.em2px(-p.y);
          d3 =
            "M" +
            XypicGlobalContext.measure.em2px(p.x - nx) +
            "," +
          ...
          for (var i = 0; i < n; i++) {
            ...
            d1 +=
              " Q" +
              XypicGlobalContext.measure.em2px(p.x + 2 * nx) +
              "," +
              XypicGlobalContext.measure.em2px(-p.y - 2 * ny);
            d2 +=
              " Q" +
              XypicGlobalContext.measure.em2px(p.x + nx) +
              "," +
        ...
        case "2":
          ...
          for (var i = 0; i < n; i++) {
            ...
            d1 +=
              " Q" +
              XypicGlobalContext.measure.em2px(p.x + 3 * nx) +
              "," +
              XypicGlobalContext.measure.em2px(-p.y - 3 * ny);
            d2 +=
              " Q" +
              XypicGlobalContext.measure.em2px(p.x + nx) +
              "," +
  ...
  _drawCurve(svg, objectForDrop, objectForConnect) {
    ...
    if (objectForConnect !== undefined) {
        ...
        case "..":
          switch (variant) {
            ...
            case "3":
              ...
              this.drawPrimitive(
                svg,
                XypicGlobalContext.measure.dottedDasharray,
              );
              break;
            ...
            switch (variant) {
              ...
              case "3":
                ...
                shifted.drawPrimitive(
                  svg,
                  XypicGlobalContext.measure.em2px(dash) +
                    " " +
                    XypicGlobalContext.measure.em2px(dash),
                );
                break;
      ...
      var shiftLen =
        (len - n * objectLen + objectLen - objectWidth) / 2 + objectBBox.l;
      var dummyContext = new DrawingContext(Shape.none, dummyEnv);
      for (var i = 0; i < n; i++) {
...
Curve.QuadBezier = class Curve_QuadBezier extends Curve {
  ...
  divide(t) {
    ...
    return [new Curve.QuadBezier(p0, p1, p2), new Curve.QuadBezier(q0, q1, q2)];
  }
  ...
  tOfIntersections(frame) {
    ...
    if (frame.isRect()) {
      ...
      tsCandidate = tsCandidate.concat(
        Curve.solutionsOfQuadEq(a2x, a1x, a0x - lx),
      );
      ...
      tsCandidate = tsCandidate.concat(
        Curve.solutionsOfQuadEq(a2y, a1y, a0y - dy),
      );
      for (var i = 0; i < tsCandidate.length; i++) {
        var t = tsCandidate[i];
        var x = px(t);
  ...
  drawPrimitive(svg, dasharray) {
    ...
    svg.createSvgElement("path", {
      d:
        "M" +
        XypicGlobalContext.measure.em2px(cp0.x) +
        "," +
        XypicGlobalContext.measure.em2px(-cp0.y) +
        " Q" +
        XypicGlobalContext.measure.em2px(cp1.x) +
        "," +
        XypicGlobalContext.measure.em2px(-cp1.y) +
        " " +
        XypicGlobalContext.measure.em2px(cp2.x) +
        "," +
        XypicGlobalContext.measure.em2px(-cp2.y),
...
Curve.CubicBezier = class Curve_CubicBezier extends Curve {
  ...
  tOfIntersections(frame) {
    ...
    if (frame.isRect()) {
      ...
      tsCandidate = tsCandidate.concat(
        Curve.solutionsOfCubicEq(a3x, a2x, a1x, a0x - lx),
      );
      for (var i = 0; i < tsCandidate.length; i++) {
        var t = tsCandidate[i];
        var y = py(t);
      ...
      tsCandidate = tsCandidate.concat(
        Curve.solutionsOfCubicEq(a3y, a2y, a1y, a0y - dy),
      );
      for (var i = 0; i < tsCandidate.length; i++) {
        var t = tsCandidate[i];
        var x = px(t);
  ...
  drawPrimitive(svg, dasharray) {
    ...
    svg.createSvgElement("path", {
      d:
        ...
        XypicGlobalContext.measure.em2px(-cp2.y) +
        " " +
        XypicGlobalContext.measure.em2px(cp3.x) +
        "," +
        XypicGlobalContext.measure.em2px(-cp3.y),
      "stroke-dasharray": dasharray,
    });
  }
...
Curve.Line = class Curve_Line {
  ...
  tOfIntersections(frame) {
    ...
    if (frame.isRect()) {
      ...
      tsCandidate = tsCandidate.concat(
        Curve.solutionsOfLinearEq(a1x, a0x - lx),
      );
      for (var i = 0; i < tsCandidate.length; i++) {
        var t = tsCandidate[i];
        var y = py(t);
      ...
      tsCandidate = tsCandidate.concat(
        Curve.solutionsOfLinearEq(a1y, a0y - dy),
      );
      for (var i = 0; i < tsCandidate.length; i++) {
        var t = tsCandidate[i];
        var x = px(t);
      ...
      switch (main) {
        ...
        case "~":
          ...
          if (len >= wave) {
            ...
            var d =
              "M" +
              XypicGlobalContext.measure.em2px(sx) +
              "," +
              XypicGlobalContext.measure.em2px(sy) +
              " Q" +
              XypicGlobalContext.measure.em2px(sx + tx + cx) +
              "," +
              XypicGlobalContext.measure.em2px(sy - ty - cy) +
              " " +
              XypicGlobalContext.measure.em2px(sx + 2 * tx) +
              "," +
              XypicGlobalContext.measure.em2px(sy - 2 * ty) +
              " T" +
              XypicGlobalContext.measure.em2px(sx + 4 * tx) +
              "," +
              XypicGlobalContext.measure.em2px(sy - 4 * ty);
            for (var i = 1; i < n; i++) {
              d +=
                " T" +
                XypicGlobalContext.measure.em2px(sx + (4 * i + 2) * tx) +
                "," +
                XypicGlobalContext.measure.em2px(sy - (4 * i + 2) * ty) +
                " T" +
                XypicGlobalContext.measure.em2px(sx + (4 * i + 4) * tx) +
                "," +
                XypicGlobalContext.measure.em2px(sy - (4 * i + 4) * ty);
            }
        ...
        case "~~":
          ...
          if (len >= wave) {
            ...
            for (var i = 0; i <= n; i++) {
              d +=
                " M" +
                XypicGlobalContext.measure.em2px(sx + 8 * i * tx) +
                "," +
                XypicGlobalContext.measure.em2px(sy - 8 * i * ty) +
                " Q" +
                XypicGlobalContext.measure.em2px(sx + (8 * i + 1) * tx + cx) +
                "," +
                XypicGlobalContext.measure.em2px(sy - (8 * i + 1) * ty - cy) +
                " " +
                XypicGlobalContext.measure.em2px(sx + (8 * i + 2) * tx) +
                "," +
                XypicGlobalContext.measure.em2px(sy - (8 * i + 2) * ty) +
                " T" +
                XypicGlobalContext.measure.em2px(sx + (8 * i + 4) * tx) +
                "," +
                XypicGlobalContext.measure.em2px(sy - (8 * i + 4) * ty);
            }
            this.drawSquigglyLineShape(svg, d, s, e, cx, cy, variant);
          }
          break;
  ...
  drawStraightLine(svg, s, e, shift, angle, t, variant, dasharray) {
    if (variant === "3") {
      ...
      svg.createSvgElement("line", {
        x1: XypicGlobalContext.measure.em2px(s.x + shift.x),
        y1: -XypicGlobalContext.measure.em2px(s.y + shift.y),
        x2: XypicGlobalContext.measure.em2px(e.x),
        y2: -XypicGlobalContext.measure.em2px(e.y),
        "stroke-dasharray": dasharray,
      });
      svg.createSvgElement("line", {
        x1: XypicGlobalContext.measure.em2px(s.x + cx + shift.x),
        y1: -XypicGlobalContext.measure.em2px(s.y + cy + shift.y),
        x2: XypicGlobalContext.measure.em2px(e.x + cx),
        y2: -XypicGlobalContext.measure.em2px(e.y + cy),
        "stroke-dasharray": dasharray,
      });
      svg.createSvgElement("line", {
        x1: XypicGlobalContext.measure.em2px(s.x - cx + shift.x),
...
CurveSegment.Line = class CurveSegment_Line extends CurveSegment {
  ...
  drawFatLine() {
    ...
    XypicGlobalContext.svgForDebug.createSvgElement("line", {
      x1: XypicGlobalContext.measure.em2px(xmin),
      y1: -XypicGlobalContext.measure.em2px(y(xmin, lmax)),
      x2: XypicGlobalContext.measure.em2px(xmax),
      y2: -XypicGlobalContext.measure.em2px(y(xmax, lmax)),
      "stroke-width": XypicGlobalContext.measure.em2px(
        0.02 * XypicGlobalContext.measure.oneem,
      ),
      stroke: "red",
    });
  }
...
CurveSegment.QuadBezier = class CurveSegment_QuadBezier extends CurveSegment {
  ...
  drawFatLine() {
    ...
    XypicGlobalContext.svgForDebug.createSvgElement("line", {
      x1: XypicGlobalContext.measure.em2px(xmin),
      y1: -XypicGlobalContext.measure.em2px(y(xmin, lmin)),
      x2: XypicGlobalContext.measure.em2px(xmax),
      y2: -XypicGlobalContext.measure.em2px(y(xmax, lmin)),
      "stroke-width": XypicGlobalContext.measure.em2px(
        0.02 * XypicGlobalContext.measure.oneem,
      ),
      stroke: "blue",
...
LastCurve.Line = class LastCurve_Line extends LastCurve {
  ...
  derivative(t) {
    return new Frame.Point(this.c.x - this.p.x, this.c.y - this.p.y);
  }
...
LastCurve.QuadBezier = class LastCurve_QuadBezier extends LastCurve {
  constructor(origBezier, tOfShavedStart, tOfShavedEnd, curveShape) {
    ...
    if (!curveShape.isNone) {
      ...
      if (tOfShavedStart > 0) {
        curveShape.sliceHole(new Range(0, tOfShavedStart));
      }
      if (tOfShavedEnd < 1) {
        curveShape.sliceHole(new Range(tOfShavedEnd, 1));
      }
    }
  }
...
LastCurve.CubicBSpline = class LastCurve_CubicBSpline extends LastCurve {
  ...
  segments() {
    ...
    for (var i = 0; i < n; i++) {
      segments[i] = new CurveSegment.CubicBezier(
        this.origBezier,
        i / n,
        (i + 1) / n,
      );
