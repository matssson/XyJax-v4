...
export function augment(klass, methods) {
  for (let name in methods) {
    if (!klass.prototype.hasOwnProperty(name)) {
      ...
      console.log(
        "WARN",
        "method " + name + " is already exists in class " + klass.name,
      );
    }
  }
}
...
augment(AST.Pos.Place, {
  toShape: function (context) {
    ...
    if (env.lastCurve.isDefined) {
      ...
      dimen = XypicGlobalContext.measure.length2em(
        place.slide.dimen.getOrElse("0"),
      );
...
augment(AST.Place.Intercept, {
  value: function (context) {
    ...
    if (!tmpEnv.lastCurve.isDefined) {
      tmpEnv.lastCurve = new LastCurve.Line(
        tmpEnv.p,
        tmpEnv.c,
        tmpEnv.p,
        tmpEnv.c,
    ...
    for (var i = 0; i < thisSegs.length; i++) {
      for (var j = 0; j < thatSegs.length; j++) {
        intersec = intersec.concat(
          CurveSegment.findIntersections(thisSegs[i], thatSegs[j]),
        );
      }
    }
...
augment(AST.ObjectBox, {
  toConnectShape: function (context) {
    ...
    if (s.x !== e.x || s.y !== e.y) {
      var shape = new Curve.Line(s, e).toShape(
        context,
        this,
...
augment(AST.ObjectBox.Cir.Cir.Segment, {
  toDropShape: function (context, x, y, r) {
    ...
    var circleSegmentShape = new Shape.CircleSegmentShape(
      x,
      y,
      sx,
      sy,
    ...
    switch (this.main) {
      case "--":
        ...
        if (convertToEllipse) {
          ...
          shape = new Shape.RectangleShape(
            x,
            y,
            left,
            right,
      ...
      case "(":
        shape = new Shape.LeftParenthesis(
          x - left,
          y + (up - down) / 2,
          up + down,
          0,
...
augment(AST.ObjectBox.Dir, {
  toDropShape: function (context) {
    ...
    switch (this.main) {
      ...
      default:
        var newdirObj = XypicGlobalContext.repositories.dirRepository.get(
          this.main,
        );
...
augment(AST.ObjectBox.Curve, {
  ...
  toConnectShape: function (context) {
    ...
    if (objectForDrop === undefined && objectForConnect === undefined) {
      objectForConnect = new AST.Object(
        List.empty,
    ...
    switch (controlPoints.length) {
      ...
      case 1:
        ...
        env.lastCurve = new LastCurve.QuadBezier(
          origBezier,
          tOfShavedStart,
          tOfShavedEnd,
          shape,
      ...
      case 2:
        var origBezier = new Curve.CubicBezier(
          s,
          controlPoints[0],
          controlPoints[1],
          e,
        ...
        shape = origBezier.toShape(context, objectForDrop, objectForConnect);
        env.lastCurve = new LastCurve.CubicBezier(
          origBezier,
          tOfShavedStart,
          tOfShavedEnd,
          shape,
      ...
      default:
        ...
        env.lastCurve = new LastCurve.CubicBSpline(
          s,
          e,
          origBeziers,
          tOfShavedStart,
...
augment(AST.Vector.Abs, {
  xy: function (context) {
    return {
      x: XypicGlobalContext.measure.length2em(this.x),
      y: XypicGlobalContext.measure.length2em(this.y),
    };
  },
  angle: function (context) {
...
augment(AST.Modifier.Shape.Rect, {
  ...
  modifyShape: function (context, objectShape, restModifiers) {
    var c = context.env.c;
    context.env.c = new Frame.Rect(c.x, c.y, {
      l: c.l,
      r: c.r,
      u: c.u,
      d: c.d,
...
augment(AST.Modifier.Shape.Alphabets, {
  preprocess: function (context, reversedProcessedModifiers) {
    var modifier = XypicGlobalContext.repositories.modifierRepository.get(
      this.alphabets,
    );
...
augment(AST.Modifier.Shape.DefineShape, {
  preprocess: function (context, reversedProcessedModifiers) {
    ...
    XypicGlobalContext.repositories.modifierRepository.put(
      this.shape,
      new AST.Modifier.Shape.CompositeModifiers(processedModifiers),
    );
  },
  modifyShape: function (context, objectShape, restModifiers) {
    return this.proceedModifyShape(context, objectShape, restModifiers);
          ...
          new AST.Command.Path.Label.At(
            new AST.Pos.Place(
              new AST.Place(
                1,
                1,
                new AST.Place.Factor(0),
                new AST.Slide(Option.empty),
              ),
            ),
...
augment(AST.Command.Ar.Form.CurveArrow, {
  toShape: function (context) {
    var env = context.env;
    var cpDist = XypicGlobalContext.measure.em2length(
      XypicGlobalContext.measure.length2em(this.dist) * 2,
    );
                    ...
                    .append(
                      new AST.ObjectBox.Curve.PosList.Pos(
                        new AST.Pos.Coord(
                          new AST.Coord.C(),
                          List.empty
                            .append(new AST.Pos.SwapPAndC(new AST.Coord.C()))
                            .append(
                              new AST.Pos.Plus(
                                new AST.Coord.Vector(
                                  new AST.Vector.Dir(this.outDirection, "3pc"),
                                ),
                              ),
                            ),
                        ),
                        ...
                        new AST.Pos.Coord(
                          ...
                          List.empty
                            ...
                            .append(
                              new AST.Pos.Plus(
                                new AST.Coord.Vector(
                                  new AST.Vector.Dir(this.inDirection, "3pc"),
                                ),
                              ),
                            ),
                        ),
...
augment(AST.Command.Ar.Form.Slide, {
  toShape: function (context) {
    context.env.segmentSlideEm = new Option.Some(
      XypicGlobalContext.measure.length2em(this.slideDimen),
    );
  },
});
...
augment(AST.Command.Path.TurningSegment, {
  toShape: function (context) {
    ...
    if (slideEm !== 0) {
      env.p = env.p.move(env.p.x - slideEm * sv.y, env.p.y + slideEm * sv.x);
      env.c = env.c.move(env.c.x - slideEm * ev.y, env.c.y + slideEm * ev.x);
      if (circle.orient === "^") {
...
augment(AST.Command.Path.Label, {
  toShape: function (context) {
    ...
    if (labelmargin !== 0) {
      ...
      if (!lastCurve.isNone) {
        angle =
          lastCurve.angle(t) + Math.PI / 2 + (labelmargin > 0 ? 0 : Math.PI);
      } else {
      ...
      if (bbox !== undefined) {
        ...
        context.appendShapeToFront(
          new Shape.TranslateShape(margin * cos, margin * sin, labelShape),
        );
      }
    } else {
...
augment(AST.Command.Xymatrix, {
  toShape: function (context) {
    ...
    origEnv.c = new Frame.Rect(origEnv.c.x, origEnv.c.y, {
      l: l,
      r: r,
      u: u,
      d: d,
...
augment(AST.Command.Xymatrix.Setup.ChangeSpacing.Row, {
  toShape: function (context) {
    ...
    env.xymatrixRowSepEm = this.addop.applyToDimen(
      ...
      XypicGlobalContext.measure.length2em(this.dimen),
    );
  },
});
...
augment(AST.Command.Xymatrix.Setup.PretendEntrySize.Height, {
  toShape: function (context) {
    context.env.xymatrixPretendEntryHeight = new Option.Some(
      XypicGlobalContext.measure.length2em(this.dimen),
    );
  },
});
...
augment(AST.Command.Xymatrix.Setup.PretendEntrySize.Width, {
  toShape: function (context) {
    context.env.xymatrixPretendEntryWidth = new Option.Some(
      XypicGlobalContext.measure.length2em(this.dimen),
    );
  },
});
...
augment(AST.Command.Xymatrix.Entry.SimpleEntry, {
  toShape: function (context) {
    var env = context.env;
    var defaultWidth = XypicGlobalContext.measure.em2length(
      env.objectmargin + env.objectwidth,
    );
    var defaultHeight = XypicGlobalContext.measure.em2length(
      env.objectmargin + env.objectheight,
    );
    var defaultSizeModifier = new AST.Modifier.AddOp(
      new AST.Modifier.AddOp.GrowTo(),
      new AST.Modifier.AddOp.VactorSize(
        new AST.Vector.Abs(defaultWidth, defaultHeight),
      ),
    );
    var margin = XypicGlobalContext.measure.em2length(env.objectmargin);
    var marginModifier = new AST.Modifier.AddOp(
      new AST.Modifier.AddOp.Grow(),
      new AST.Modifier.AddOp.VactorSize(new AST.Vector.Abs(margin, margin)),
    );
    var modifiers = this.modifiers
...
augment(AST.Command.Xymatrix.Entry.EmptyEntry, {
  toShape: function (context) {
    ...
    var modifiers = env.xymatrixEntryModifiers
      .prepend(defaultSizeModifier)
      .prepend(marginModifier);
    return new AST.Object(modifiers, new AST.ObjectBox.Empty()).toDropShape(
      context,
    );
  },
});
...
augment(AST.Command.Twocell, {
  toShape: function (context) {
    ...
    subEnv.twocellmodmapobject =
      ...
      new AST.Object(List.empty, new AST.ObjectBox.Dir("", "|"));
    subEnv.twocellhead =
      origEnv.twocellhead ||
      new AST.Object(List.empty, new AST.ObjectBox.Dir("", ">"));
    subEnv.twocelltail =
...
augment(AST.Command.Twocell.Hops2cell, {
  toShape: function (context, arrow) {
    ...
    var m = new Frame.Point(s.x + dx * 0.5, s.y + dy * 0.5);
...
augment(AST.Command.Twocell.Twocell, {
  ...
  getDefaultCurvature: function () {
    return 3.5 * XypicGlobalContext.measure.lineElementLength;
  },
  hasUpperTips: true,
  hasLowerTips: true,
});
...
augment(AST.Command.Twocell.UpperTwocell, {
  ...
  toLowerCurveShape: function (
    ...
    if (shavedS.x !== shavedE.x || shavedS.y !== shavedE.y) {
      context.env.lastCurve = new LastCurve.Line(
        shavedS,
        shavedE,
        s,
        e,
  ...
  getDefaultCurvature: function () {
    return 7 * XypicGlobalContext.measure.lineElementLength;
  },
  hasUpperTips: true,
  hasLowerTips: false,
});
...
augment(AST.Command.Twocell.CompositeMap, {
  ...
  getUpperLabelPosition: function (s, e, midPoint, curvatureEm, ncos, nsin) {
    ...
    return new Frame.Point(e.x + 0.5 * dx, e.y + 0.5 * dy);
  },
  getLowerLabelPosition: function (s, e, midPoint, curvatureEm, ncos, nsin) {
    var dx = midPoint.x + curvatureEm * ncos - s.x;
    var dy = midPoint.y + curvatureEm * nsin - s.y;
    var l = Math.sqrt(dx * dx + dy * dy);
    return new Frame.Point(s.x + 0.5 * dx, s.y + 0.5 * dy);
  },
  getUpperLabelAngle: function (
    antiClockwiseAngle,
    s,
  ...
  getMidBoxSize: function () {
    return 0.5 * XypicGlobalContext.measure.lineElementLength;
  },
  getDefaultCurvature: function () {
    return 3.5 * XypicGlobalContext.measure.lineElementLength;
  },
...
augment(AST.Command.Twocell.Switch.SetCurvature, {
  setup: function (context) {
    ...
    if (this.nudge.isOmit) {
      ...
      env.twocellCurvatureEm = new Option.Some(
        this.nudge.number * XypicGlobalContext.measure.lineElementLength,
      );
    }
  },
});
...
augment(AST.Command.Twocell.Label, {
  ...
  getDefaultLabelOffset: function () {
    return XypicGlobalContext.measure.lineElementLength;
  },
});
...
augment(AST.Command.Includegraphics.Attr.Width, {
  setup: function (context) {
    var env = context.env;
    env.includegraphicsWidth = new Option.Some(
      XypicGlobalContext.measure.length2em(this.dimen),
    );
  },
});
...
augment(AST.Command.Includegraphics.Attr.Height, {
  setup: function (context) {
    var env = context.env;
    env.includegraphicsHeight = new Option.Some(
      XypicGlobalContext.measure.length2em(this.dimen),
    );
  },
});
