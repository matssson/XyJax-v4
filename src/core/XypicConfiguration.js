...
function parseXypic(texParser, xyParser, mmlKind) {
  ...
  if (result.successful) {
    ...
    const mml = texParser.create(
      mmlKind,
      { "data-cmd-id": commandId, "data-text-mml-ids": textMmlIdsJson },
      textMmls,
    );
    return mml;
  } else {
    ...
    const text =
      parseContext.lastNoSuccess.msg +
...
XypicMethods.xymatrixbox = function (parser, name, type) {
  const mml = parseXypic(
    parser,
    XyParser.xymatrixbox(),
    AST.xypic.prototype.kind,
  );
  parser.Push(mml);
};
...
XypicMethods.newdir = function (parser, name, type) {
  const mml = parseXypic(
    parser,
    XyParser.newdir(),
    AST.xypic.newdir.prototype.kind,
  );
  parser.Push(mml);
};
...
XypicMethods.xyincludegraphics = function (parser, name, type) {
  const mml = parseXypic(
    parser,
    XyParser.includegraphics(),
    AST.xypic.includegraphics.prototype.kind,
  );
  parser.Push(mml);
};
...
const XypicCommandMap = new CommandMap(
  "xypic-command",
  {
...
const XypicEnvironmentMap = new EnvironmentMap(
  "xypic-environment",
...
const XypicConfiguration = Configuration.create("xypic", {
  handler: {
    macro: ["xypic-command"],
...
ModifierRepository.embeddedModifierMap = {
  ...
  lightgoldenrodyellow: new AST.Modifier.Shape.ChangeColor(
