...
var felem = function (x) {
  return fun(Parsers.elem(x));
};
var lit = Parsers.literal;
var regex = Parsers.regex;
var regexLit = Parsers.regexLiteral;
var flit = function (x) {
  return fun(Parsers.literal(x));
};
var seq = Parsers.seq;
var or = Parsers.or;
var rep = function (x) {
  return Parsers.lazyParser(x)().rep();
};
var rep1 = function (x) {
  return Parsers.lazyParser(x)().rep1();
};
var opt = function (x) {
  return Parsers.lazyParser(x)().opt();
};
var not = function (x) {
  return Parsers.not(Parsers.lazyParser(x));
};
...
var grammar = {
  ...
  xymatrixbox: memo(function () {
    return p.xymatrix().to(function (m) {
      return new AST.PosDecor(
        new AST.Pos.Coord(new AST.Coord.C(), List.empty),
        new AST.Decor(List.empty.append(m)),
      );
    });
  }),
  ...
  nonemptyPos: memo(function () {
    return or(seq(p.nonemptyCoord, rep(p.pos2)), seq(p.coord, rep1(p.pos2))).to(
      function (cps) {
        return new AST.Pos.Coord(cps.head, cps.tail);
      },
    );
  ...
  pos2: memo(function () {
    return or(
      lit("+")
        .andr(p.coord)
        .to(function (c) {
          return new AST.Pos.Plus(c);
        }),
      lit("-")
        .andr(p.coord)
        .to(function (c) {
          return new AST.Pos.Minus(c);
        }),
      lit("!")
        .andr(p.coord)
        .to(function (c) {
          return new AST.Pos.Skew(c);
        }),
      lit(".")
        .andr(p.coord)
        .to(function (c) {
          return new AST.Pos.Cover(c);
        }),
      lit(",")
        .andr(p.coord)
        .to(function (c) {
          return new AST.Pos.Then(c);
        }),
      lit(";")
        .andr(p.coord)
        .to(function (c) {
          return new AST.Pos.SwapPAndC(c);
        }),
      lit("::")
        .andr(p.coord)
        .to(function (c) {
          return new AST.Pos.SetYBase(c);
        }),
      lit(":")
        .andr(p.coord)
        .to(function (c) {
          return new AST.Pos.SetBase(c);
        }),
      lit("**")
        .andr(p.object)
        .to(function (o) {
          return new AST.Pos.ConnectObject(o);
        }),
      lit("*")
        .andr(p.object)
        .to(function (o) {
          return new AST.Pos.DropObject(o);
        }),
      lit("?")
        .andr(p.place)
        .to(function (o) {
          return new AST.Pos.Place(o);
        }),
      lit("@+")
        .andr(p.coord)
        .to(function (c) {
          return new AST.Pos.PushCoord(c);
        }),
      lit("@-")
        .andr(p.coord)
        .to(function (c) {
          return new AST.Pos.EvalCoordThenPop(c);
        }),
      lit("@=")
        .andr(p.coord)
        .to(function (c) {
          return new AST.Pos.LoadStack(c);
        }),
      lit("@@")
        .andr(p.coord)
        .to(function (c) {
          return new AST.Pos.DoCoord(c);
        }),
      lit("@i").to(function () {
        return new AST.Pos.InitStack();
      }),
      lit("@(").to(function () {
        return new AST.Pos.EnterFrame();
      }),
      lit("@)").to(function () {
        return new AST.Pos.LeaveFrame();
      }),
      lit("=:")
        .andr(flit('"'))
        .andr(p.id)
        .andl(felem('"'))
        .to(function (id) {
          return new AST.Pos.SaveBase(id);
        }),
      lit("=@")
        .andr(flit('"'))
        .andr(p.id)
        .andl(felem('"'))
        .to(function (id) {
          return new AST.Pos.SaveStack(id);
        }),
      lit("=")
        .andr(flit('"'))
        .andr(p.id)
      ...
      lit("=")
        ...
        .andl(felem('"'))
        .to(function (mid) {
          return new AST.Pos.SaveMacro(mid.head, mid.tail);
        }),
      p.xyimport,
    );
  }),
  ...
  nonemptyCoord: memo(function () {
    return or(
      ...
      lit("p").to(function () {
        return new AST.Coord.P();
      }),
      lit("x").to(function () {
        return new AST.Coord.X();
      }),
      lit("y").to(function () {
        return new AST.Coord.Y();
      }),
      p.vector().to(function (v) {
        return new AST.Coord.Vector(v);
      }),
      lit('"')
        .andr(p.id)
        .andl(felem('"'))
        .to(function (id) {
          return new AST.Coord.Id(id);
        }),
      lit("{")
        .andr(p.posDecor)
        .andl(flit("}"))
        .to(function (pd) {
          return new AST.Coord.Group(pd);
        }),
      lit("s")
        .andr(fun(regexLit(/^\d/)))
        .to(function (n) {
      ...
      lit("[")
        ...
        .andl(flit("]"))
        .to(function (prc) {
          return new AST.Coord.DeltaRowColumn(
            prc.head.head,
            prc.head.tail,
            prc.tail,
          );
  ...
  vector: memo(function () {
    return or(
      ...
      lit("0").to(function (x) {
        return new AST.Vector.Abs("0mm", "0mm");
      }),
      function () {
        return p
          .corner()
          .and(
            fun(
              Parsers.opt(fun(lit("(").andr(p.factor).andl(flit(")")))).to(
                function (f) {
  ...
  corner: memo(function () {
    return or(
      regexLit(/^(CL|LC)/).to(function () {
        return new AST.Corner.CL();
      }),
      regexLit(/^(CR|RC)/).to(function () {
        return new AST.Corner.CR();
      }),
      regexLit(/^(CD|DC)/).to(function () {
        return new AST.Corner.CD();
      }),
      regexLit(/^(CU|UC)/).to(function () {
        return new AST.Corner.CU();
      }),
      regexLit(/^(LD|DL)/).to(function () {
        return new AST.Corner.LD();
      }),
      regexLit(/^(RD|DR)/).to(function () {
        return new AST.Corner.RD();
      }),
      regexLit(/^(LU|UL)/).to(function () {
        return new AST.Corner.LU();
      }),
      regexLit(/^(RU|UR)/).to(function () {
        return new AST.Corner.RU();
      }),
      lit("L").to(function () {
        return new AST.Corner.L();
      }),
      lit("R").to(function () {
        return new AST.Corner.R();
      }),
      lit("D").to(function () {
        return new AST.Corner.D();
      }),
      lit("U").to(function () {
        return new AST.Corner.U();
      }),
      lit("E").to(function () {
        return new AST.Corner.NearestEdgePoint();
      }),
      lit("P").to(function () {
        return new AST.Corner.PropEdgePoint();
      }),
      lit("A").to(function () {
        return new AST.Corner.Axis();
      }),
    );
  }),
  ...
  nonemptyCir: memo(function () {
    return p
      ...
      .and(p.diag)
      .to(function (dod) {
        return new AST.ObjectBox.Cir.Cir.Segment(
          dod.head.head,
          dod.head.tail,
          dod.tail,
        );
  ...
  curveOption: memo(function () {
    return or(
      lit("p").to(function () {
        return new AST.ObjectBox.Curve.Modifier.p();
      }),
      lit("P").to(function () {
        return new AST.ObjectBox.Curve.Modifier.P();
      }),
      lit("l").to(function () {
        return new AST.ObjectBox.Curve.Modifier.l();
      }),
      lit("L").to(function () {
        return new AST.ObjectBox.Curve.Modifier.L();
      }),
      lit("c").to(function () {
        return new AST.ObjectBox.Curve.Modifier.c();
      }),
      lit("C").to(function () {
        return new AST.ObjectBox.Curve.Modifier.C();
      }),
      lit("pc").to(function () {
        return new AST.ObjectBox.Curve.Modifier.pc();
      }),
      lit("pC").to(function () {
        return new AST.ObjectBox.Curve.Modifier.pC();
      }),
      ...
      lit("lc").to(function () {
        return new AST.ObjectBox.Curve.Modifier.lc();
      }),
      lit("lC").to(function () {
        return new AST.ObjectBox.Curve.Modifier.lC();
      }),
      ...
      lit("cC").to(function () {
        return new AST.ObjectBox.Curve.Modifier.cC();
      }),
    );
  }),
  ...
  curvePoslist: memo(function () {
    return or(
      ...
      p
        ...
        .and(p.curvePoslist2)
        .to(function (pps) {
          return pps.tail.prepend(
            new AST.ObjectBox.Curve.PosList.Pos(pps.head),
          );
        }),
  ...
  addOp: memo(function () {
    return or(
      lit("+=").to(function () {
        return new AST.Modifier.AddOp.GrowTo();
      }),
      lit("-=").to(function () {
        return new AST.Modifier.AddOp.ShrinkTo();
      }),
      lit("+").to(function () {
        return new AST.Modifier.AddOp.Grow();
      }),
      lit("-").to(function () {
        return new AST.Modifier.AddOp.Shrink();
      }),
      lit("=").to(function () {
        return new AST.Modifier.AddOp.Set();
      }),
    );
  }),
  ...
  shape: memo(function () {
    return or(
      lit(".").to(function () {
        return new AST.Modifier.Shape.Point();
      }),
      p.frameShape,
      ...
      success("rect").to(function () {
        return new AST.Modifier.Shape.Rect();
      }),
    );
  }),
  ...
  nonemptyDiag: memo(function () {
    return or(
      regexLit(/^(ld|dl)/).to(function (x) {
        return new AST.Diag.LD();
      }),
      regexLit(/^(rd|dr)/).to(function (x) {
        return new AST.Diag.RD();
      }),
      regexLit(/^(lu|ul)/).to(function (x) {
        return new AST.Diag.LU();
      }),
      regexLit(/^(ru|ur)/).to(function (x) {
        return new AST.Diag.RU();
      }),
      lit("l").to(function (x) {
        return new AST.Diag.L();
      }),
      lit("r").to(function (x) {
        return new AST.Diag.R();
      }),
      lit("d").to(function (x) {
        return new AST.Diag.D();
      }),
      lit("u").to(function (x) {
        return new AST.Diag.U();
      }),
    );
  }),
  ...
  arrowForm: memo(function () {
    return or(
      ...
      lit("@")
        ...
        .andl(flit("/"))
        .to(function (dd) {
          return new AST.Command.Ar.Form.CurveArrow(
            dd.head,
      ...
      lit("@")
        ...
        .to(function (vtct) {
          ...
          if (vtct.tail.isDefined) {
            ...
            return new AST.Command.Ar.Form.BuildArrow(
              variant,
              tct.tail,
              tct.stem,
              tct.head,
    ...
    return lit("{")
      ...
      .to(function (pcp) {
        ...
        if (!maybeStem.isDefined && !maybeHead.isDefined) {
          ...
          stem = maybeStem.getOrElse(
            new AST.Command.Ar.Form.Conn.Connchars(""),
          );
  ...
  path: memo(function () {
    return p
      .path2(List.empty /* initial failure continuation */)
      .to(function (ps) {
  ...
  segment: memo(function () {
    return p
      ...
      .and(p.labels)
      .to(function (psl) {
        return new AST.Command.Path.Segment(
          psl.head.head,
          psl.head.tail,
          psl.tail,
        );
  ...
  label: memo(function () {
    return or(
      seq("^", p.anchor, p.it, p.alias).to(function (aia) {
        return new AST.Command.Path.Label.Above(
          new AST.Pos.Place(aia.head.head.tail),
          aia.head.tail,
          aia.tail,
        );
      }),
      seq("_", p.anchor, p.it, p.alias).to(function (aia) {
        return new AST.Command.Path.Label.Below(
          new AST.Pos.Place(aia.head.head.tail),
          aia.head.tail,
          aia.tail,
        );
      ...
      seq("|", p.anchor, p.it, p.alias).to(function (aia) {
        return new AST.Command.Path.Label.At(
          new AST.Pos.Place(aia.head.head.tail),
          aia.head.tail,
          aia.tail,
        );
    ...
    return rep(
      ...
      .to(function (si) {
        return new AST.Object(
          si.head.concat(si.tail.modifiers),
          si.tail.object,
        );
      });
  }),
    ...
    return or(
      regexLit(/^[0-9a-zA-Z]/)
        ...
        .to(function (cc) {
          ...
          return new AST.Object(
            List.empty,
            p.toMath("\\labelstyle " + c, createTextNode),
          );
        }),
      regexLit(/^(\\[a-zA-Z][a-zA-Z0-9]*)/)
        .and(p.textNodeCreator)
        .to(function (cc) {
          var c = cc.head;
          var createTextNode = cc.tail;
      ...
      lit("{")
        ...
        .to(function (tc) {
          ...
          return new AST.Object(
            List.empty,
            p.toMath("\\labelstyle " + t, createTextNode),
          );
        }),
      lit("*").andr(p.object),
      lit("@")
        ...
        or(
          ...
          lit("@!")
            ...
            .to(function (cd) {
              ...
              switch (cd.head) {
                case "R":
                  return new AST.Command.Xymatrix.Setup.PretendEntrySize.Height(
                    dimen,
                  );
                case "C":
                  return new AST.Command.Xymatrix.Setup.PretendEntrySize.Width(
                    dimen,
                  );
                default:
                  return new AST.Command.Xymatrix.Setup.PretendEntrySize.HeightAndWidth(
                    dimen,
                  );
              }
            }),
          lit("@!").andr(
            fun(
              or(
                elem("R").to(function () {
                  return new AST.Command.Xymatrix.Setup.FixGrid.Row();
                }),
                elem("C").to(function () {
                  return new AST.Command.Xymatrix.Setup.FixGrid.Column();
                }),
              )
                .opt()
                .to(function (rc) {
                  return rc.getOrElse(
                    new AST.Command.Xymatrix.Setup.FixGrid.RowAndColumn(),
                  );
                }),
            ),
          ),
          lit("@")
            ...
            .to(function (cod) {
              ...
              switch (cod.head.head) {
                case "M":
                  return new AST.Command.Xymatrix.Setup.AdjustEntrySize.Margin(
                    addop,
                    dimen,
                  );
                case "W":
                  return new AST.Command.Xymatrix.Setup.AdjustEntrySize.Width(
                    addop,
                    dimen,
                  );
                case "H":
                  return new AST.Command.Xymatrix.Setup.AdjustEntrySize.Height(
                    addop,
                    dimen,
                  );
                case "L":
                  return new AST.Command.Xymatrix.Setup.AdjustLabelSep(
                    addop,
                    dimen,
                  );
              }
          ...
          lit("@*")
            ...
            .and(p.size)
            .to(function (os) {
              return new AST.Command.Xymatrix.Setup.AddModifier(
                new AST.Modifier.AddOp(os.head, os.tail),
              );
            }),
          lit("@")
            .andr(
            ...
            .to(function (cod) {
              ...
              switch (cod.head.head) {
                case "R":
                  return new AST.Command.Xymatrix.Setup.ChangeSpacing.Row(
                    addop,
                    dimen,
                  );
                case "C":
                  return new AST.Command.Xymatrix.Setup.ChangeSpacing.Column(
                    addop,
                    dimen,
                  );
                default:
                  return new AST.Command.Xymatrix.Setup.ChangeSpacing.RowAndColumn(
                    addop,
                    dimen,
                  );
              }
    ...
    return or(
      ...
      p
        ...
        .to(function (mopd) {
          ...
          return new AST.Command.Xymatrix.Entry.SimpleEntry(
            modifiers,
            objbox,
            decor,
          );
    ...
    return or(
      ...
      or(
        ...
        .to(function (chtc) {
          ...
          var textObject = new AST.Object(
            List.empty,
            p.toMath("\\labelstyle " + cht.tail, createTextNode),
          );
    ...
    return or(
      ...
      regexLit(/^(~[`'])/)
        ...
        .to(function (wo) {
          ...
          return new AST.Command.Twocell.Switch.ChangeHeadTailObject(
            what,
            wo.tail,
          );
        }),
      regexLit(/^(~[\^_]?)/)
        ...
        .to(function (wso) {
          ...
          return new AST.Command.Twocell.Switch.ChangeCurveObject(
            what,
            spacer,
            maybeObject,
          );
    ...
    return or(
      regexLit(/^[0-9a-zA-Z]/)
        ...
        .to(function (cc) {
          var c = cc.head;
          var createTextNode = cc.tail;
          var obj = new AST.Object(
            List.empty,
            p.toMath("\\twocellstyle " + c, createTextNode),
          );
      ...
      lit("{")
        ...
        .to(function (ntc) {
          ...
          var obj = new AST.Object(
            List.empty,
            p.toMath("\\twocellstyle " + nt.tail, createTextNode),
          );
  ...
  twocellArrow: memo(function () {
    return or(
      lit("{")
        ...
        .andl(flit("}"))
        .to(function (te) {
          return new AST.Command.Twocell.Arrow.WithOrientation(
            te.head,
            te.tail,
          );
        }),
      ...
      success("no arrow label")
        .andr(p.textNodeCreator)
        .to(function (createTextNode) {
          // TODO 無駄な空描画処理をなくす。
          return new AST.Command.Twocell.Arrow.WithOrientation(
            "",
            new AST.Object(
              List.empty,
              p.toMath("\\twocellstyle{}", createTextNode),
            ),
          );
        }),
    );
    ...
    return or(
      ...
      p
        ...
        .to(function (tc) {
          ...
          return new AST.Object(
            List.empty,
            p.toMath("\\twocellstyle " + t, createTextNode),
          );
        }),
    );
  }),
  ...
  newdir: memo(function () {
    return lit("{")
      ...
      .andl(flit("}"))
      .to(function (mc) {
        return new AST.Command.Newdir(
          mc.head,
          new AST.ObjectBox.CompositeObject(mc.tail),
        );
      });
  }),
    ...
    return lit("\\xyimport")
      ...
      .to(function (whog) {
        ...
        if (graphics.isIncludegraphics !== undefined) {
          return new AST.Pos.Xyimport.Graphics(
            w,
            h,
            xOffset,
            yOffset,
          ...
          return new AST.Pos.Xyimport.TeXCommand(
            w,
            h,
            xOffset,
            yOffset,
